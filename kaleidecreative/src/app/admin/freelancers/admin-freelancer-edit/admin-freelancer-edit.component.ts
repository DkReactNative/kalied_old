...
@Component({
  selector: 'app-admin-freelancer-edit',
  templateUrl: './admin-freelancer-edit.component.html',
  styleUrls: ['./admin-freelancer-edit.component.css']
})
export class AdminFreelancerEditComponent implements OnInit {
  user;
  globalSettings;
  globals = environment;
  ...
  newRecordSubmit = async (formData) => {
    ...
    await this.myservices.put('admin/userManage/'+this.user_id, newFormData,response =>{
      ...
      if (response.status == 1) {
        this.router.navigate(['/admin/freelancer']);
